...
class AuthCodeFlowStrategy {
    ...
    async authenticate(request, sessionContext) {
        ...
        if (!session) {
            ...
            await redirectToAuthPage.redirectToAuthPage({ config, logger, api }, request, shop);
        }
        ...
        return handleClientError.handleClientErrorFactory({
            ...
            onError: async ({ session, error }) => {
                if (error.response.code === 401) {
                    throw await redirectToAuthPage.redirectToAuthPage({ api, config, logger }, request, session.shop);
                }
            },
        });
    }
    async ensureInstalledOnShop(request) {
        ...
        if (!offlineSession) {
            ...
            if (isEmbedded) {
                redirectWithExitiframe.redirectWithExitIframe({ api, config, logger }, request, shop);
            }
            else {
                throw await beginAuth.beginAuth({ api, config, logger }, request, false, shop);
            }
        }
    ...
    async handleAuthBeginRequest(request, shop) {
        ...
        if (config.isEmbeddedApp &&
            ...
            throw redirectWithExitiframe.redirectWithExitIframe({ api, config, logger }, request, shop);
        }
        else {
            throw await beginAuth.beginAuth({ api, config, logger }, request, false, shop);
        }
